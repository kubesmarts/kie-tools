...
"${MAVEN_HOME}"/bin/mvn -B ${MAVEN_ARGS_APPEND} \
  ...
  com.redhat.quarkus.platform:quarkus-maven-plugin:"${QUARKUS_PLATFORM_VERSION}":create ${QUARKUS_CREATE_ARGS} \
...
"${MAVEN_HOME}"/bin/mvn -B ${MAVEN_ARGS_APPEND} \
  ...
  clean dependency:go-offline com.redhat.quarkus.platform:quarkus-maven-plugin:"${QUARKUS_PLATFORM_VERSION}":go-offline install
