...
const imageUrl = `${env.oslDataIndexEphemeralImage.registry}/${env.oslDataIndexEphemeralImage.account}/${env.oslDataIndexEphemeralImage.name}`;

// Replace the whole string between quotes ("") with the image name
imageYaml = imageYaml.replace(/(?<=")(.*logic-data-index-ephemeral.*)(?=")/gm, imageUrl);

// Write file and then rename it to match the image name
fs.writeFileSync(originalYamlPath, imageYaml);
fs.renameSync(originalYamlPath, path.join(resourcesPath, `${env.oslDataIndexEphemeralImage.name}-image.yaml`));
